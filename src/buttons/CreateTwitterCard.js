...
function CreateTwitterCard() {
    ...
    const createTwitterProfileCard = () => {
        const uid = uuid();
        ...
        localUserPage.cards[uid] =
        {
            ...
            details:{
                ...
                url: "http://www.twitter.com/" + profileUsername
            }
        }
    ...
    const createTwitterPostCard = () => {
        const uid = uuid();
        let localUserPage = userPage;
        localUserPage.cards[uid] =
        {
            id:uid,
            type:'TwitterPostCard',
            details:{
                url: postUrl
            }
        }
    ...
    return (
        ...
        <Dialog centered open={show} onClose={handleClose}>
            ...
            <DialogActions>
                ...
                <Button onClick={() => {socialOption == 'profile' ? (createTwitterProfileCard()):(createTwitterPostCard())}} style={{backgroundColor:'#4169e1', color:'white'}}>Add</Button>
            </DialogActions>
        </Dialog>
        <Button onClick={() => handleShow()} style={{marginBottom: 10, backgroundColor:'#00acee', color:'white'}} variant="contained" startIcon={<TwitterIcon />}>Twitter</Button>
        </>
    )
}

export default CreateTwitterCard
